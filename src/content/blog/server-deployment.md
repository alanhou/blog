...
seriesOrder: 3
title:
  en: "Chapter 3: Server Deployment"
  zh: "第3章 服务器部署"
description:
  en: "Deploy Odoo 12 in production with Nginx reverse proxy, SSL certificates, system services, and Docker containers."
  zh: "使用Nginx反向代理、SSL证书、系统服务和Docker容器部署Odoo 12生产环境。"
date: 2019-05-13
tags: ["odoo", "deployment", "nginx", "docker", "ssl"]
...
在[第一章 安装Odoo开发环境](/blog/installing-odoo-development-environment)（从源码轻松安装Odoo一节），以及[第二章 管理Odoo服务器实例](/blog/managing-odoo-server-instances)（标准化你的实例目录布局一节）中，我们探讨了开发环境的设置。生产环境的要求与其会稍有不同。本章讲解Odoo部署的具体内容。
...
- 部署服务器运行着Debian Stretch（但使用做过一些修改的衍生发行版应该也可以运行，如Ubuntu，更多信息请参见[第一章 安装Odoo开发环境](/blog/installing-odoo-development-environment)）。
...
大部分的内容和[第一章 安装Odoo开发环境](/blog/installing-odoo-development-environment)中所描述的是一样的，但也有一些关键的区别。
...
在[第一章 安装Odoo开发环境](/blog/installing-odoo-development-environment)中，我们了解了如何将实例配置保存到一个文件中。很多参数我们都使用了默认值，并且如果你照着[第二章 管理Odoo服务器实例](/blog/managing-odoo-server-instances)中*标准化你的实例目录布局*以及本章中*安装Odoo来供生产使用*小节来安装生产环境的话，你应该会在生产环境中有着相同的配置文件。这一节展示如何获取一个适合于正式环境的配置文件。
...
这一节中的使用的大部分参数都在[第一章 安装Odoo开发环境](/blog/installing-odoo-development-environment)的*在一个文件中存储实例配置*中进行了讲解。
...
我们假定你启动了Odoo实例并且已安装website应用。如果你不知道如何在Odoo中安装应用，请参见[第二章 管理Odoo服务器实例](/blog/managing-odoo-server-instances)中的*安装并升级本地插件模块*一节。
...
1. 激活开发者模式，因为CDN配置仅在激活了开发者模式之后才会显示。参照[第一章 安装Odoo开发环境](/blog/installing-odoo-development-environment)中*激活Odoo开发者工具*一节。
