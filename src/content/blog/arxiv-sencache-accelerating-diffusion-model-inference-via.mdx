...
在Wan 2.1、CogVideoX和LTX-Video上的实验验证表明,在相同计算预算下,SenCache相比现有缓存方法实现了更优的视觉质量。
...
SenCache通过分析模型输出$\mathbf{f}_\theta(\mathbf{z}_t, t)$对噪声潜变量$\mathbf{z}_t$和时间步$t$扰动的敏感度来解决这一问题。
