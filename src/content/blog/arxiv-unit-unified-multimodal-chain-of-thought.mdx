...
image: "/arxiv-visuals/arxiv-unit-unified-multimodal-chain-of-thought.png"
---
...
UniT为统一多模态模型引入了范式转变,通过测试时扩展实现迭代优化。
虽然现有的统一架构能够处理理解和生成任务,但它们通常在单次前向传播中产生输出,缺乏自我纠正能力。
本文解决了将测试时扩展(在语言模型中已被证明有效)扩展到多模态场景的挑战,在这些场景中,复杂的空间推理、物体交互和演化的指令需要分解和验证。
UniT结合了智能体数据合成、统一训练和灵活推理,激发了验证、子目标分解和内容记忆等认知行为,确立了顺序思维链推理相比并行采样方法提供更优的可扩展性。
