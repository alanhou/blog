...
image: "/arxiv-visuals/arxiv-reinforced-attention-learning.png"
---
...
通过强化学习（RL）进行后训练已经通过测试时扩展大幅提升了大语言模型（LLM）的推理能力。
然而，
将这种范式通过冗长的推理过程扩展到多模态LLM（MLLM）时，对感知任务的提升有限，甚至可能降低性能。
我们提出了强化注意力学习（RAL），这是一种策略梯度框架，直接优化内部注意力分布而非输出token序列。
