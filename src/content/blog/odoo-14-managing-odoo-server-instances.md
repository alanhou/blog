...
title:
  ...
  zh: "第2章 管理Odoo服务端实例"
description:
  en: "Configure addon paths, manage module lists, standardize directory layouts, and handle addon installation and upgrades in Odoo 14."
  zh: "配置插件路径、管理模块列表、标准化目录布局，以及处理Odoo 14中的插件安装和升级。"
date: 2021-04-26
tags: ["odoo", "odoo14", "server", "instance-management", "modules"]
image: "https://images.unsplash.com/photo-1517694712202-14dd9538aa97?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&q=80&w=1080"
---

:::zh
在[第一章 安装Odoo开发环境](/blog/odoo-14-installing-odoo-development-environment)中，我们学习了如何使用源码中所带的标准核心插件配置Odoo实例。本章主要讲解如何对Odoo实例添加非核心或自定义插件。在Odoo中，可以通过多个目录加载插件。此外，推荐使用单独的目录加载第三方插件或自定义的插件，以避免与Odoo核心模块产生冲突。甚至Odoo企业版也是一种类型的插件目录，需要像加载普通插件目录那样对其进行加载。
...
这一部分假定你已经准备好了实例并生成了配置文件，如在[第一章 安装Odoo开发环境](/blog/odoo-14-installing-odoo-development-environment)*在一个文件中存储实例配置*一节所描述。Odoo的源码存放在~/odoo-dev/odoo中，而配置文件存放在~/odoo-dev/myodoo.cfg中。
...
在[第三章 创建Odoo插件模块](/blog/odoo-14-creating-odoo-add-on-modules)中，我们会来看如何编写你自己的模块。同时，这里有一个生成内容来满足Odoo要求的快捷版黑科技：
...
有关配置文件，我们仅展示了这里需要设置的最小化选项，但很明显可以做更多设置，例如数据库名、数据库过滤器或项目所监听的端口。有关这一话题的更多信息，请参见[第一章 安装Odoo开发环境](/blog/odoo-14-installing-odoo-development-environment)。
...
如果你喜欢这种方法，我们建议你尝试[第三章 服务器部署](/blog/odoo-14-creating-odoo-add-on-modules)中的使用 Docker 运行 Odoo 一部分的内容。
...
> 📝通过使用命令行，你可以快速地修改配置选项。也可以在Terminal中查看实时日志。所有可用选项可参见[第一章 安装Odoo开发环境](/blog/odoo-14-installing-odoo-development-environment)，或使用-help命令来查看所有的选项及各个选项的描述。
...
激活开发者模式来查看模块的技术名称。如果你不知道如何激活开发者模式，请参见[第一章 安装Odoo开发环境](/blog/odoo-14-installing-odoo-development-environment)：
...
2. 从Python源代码中加载模型定义并在必要时更新数据库结构（参见[第四章 应用模型](/blog/odoo-14-application-models)了解更多信息）
3. 加载插件的数据文件并在必要时更新数据库内容（参见[第六章 管理模块数据](/blog/odoo-14-module-data)了解更多信息）
4. 如果实例中启用了演示数据则安装插件演示数据
5. 如果存在，运行插件postinit钩子
6. 运行对插件视图定义的验证
7. 如果启用了演示数据及测试，运行该插件的测试（参见[第十八章 自动化测试用例](18.md)了解更多信息）
8. 在数据库中更新模块状态
9. 从插件的翻译文件中更新数据库中的翻译（参见[第十一章 国际化](/blog/odoo-14-internationalization)了解更多信息）

> 📝preinit和postinit钩子分别使用pre_init_hook和post_init_hook键名在__manifest__.py文件中定义。这些钩子用于在插件模块的安装之前及之后触发Python函数。参见[第三章 创建Odoo插件模块](/blog/odoo-14-creating-odoo-add-on-modules)了解更多有关 init 钩子的知识。
...
1. 如果存在，先运行插件模块的预迁移步骤（参见[第六章 管理模块数据](/blog/odoo-14-module-data)了解更多信息）
2. 从Python源码中加载模型定义并在必要时更新数据库结构（参见[第四章 应用模型](/blog/odoo-14-application-models)了解更多信息）
3. 加载插件的数据文件并在必要时更新数据库内容（参见[第六章 管理模块数据](/blog/odoo-14-module-data)了解更多信息）
4. 如果实例中启用了演示数据更新插件演示数据
5. 如果模块有任何迁移方法的话，运行插件模块的后置迁移步骤（参见[第六章 管理模块数据](/blog/odoo-14-module-data)了解更多信息）
6. 运行对插件视图定义的验证
7. 如果启用了演示数据并启用了测试，运行该插件的测试（参见[第十八章 自动化测试用例](18.md)了解更多信息）
8. 在数据库中更新模块状态
9. 从插件的翻译文件中更新数据库中的翻译（参见[第十一章 国际化](/blog/odoo-14-internationalization)了解更多信息）

> 📝注意更新未安装的插件模块时什么也不会做。但是安装已安装的插件模块会重新安装该插件，这会通过一些包含数据的数据文件产生一些预期外的问题，这些文件应由用户进行更新而非在常规的模块升级处理时进行更新（参见[第六章 管理模块数据](/blog/odoo-14-module-data)中使用noupdate和forcecreate标记部分的内容）。通过用户界面不存在错误的风险，但通过命令行时则有可能发生。
...
2. 如果是生产实例请进行备份（参见[第一章 安装Odoo开发环境](/blog/odoo-14-installing-odoo-development-environment)中*管理Odoo服务端数据库*一节）。
...
如果更新这一步崩溃了，参见[第一章 安装Odoo开发环境](/blog/odoo-14-installing-odoo-development-environment)*从源码更新Odoo*一节获取恢复的操作指南。记住要保持首先在生产数据库的拷贝上进行测试。
...
不应在生产数据库直接使用该分支，因此先创建一个带有生产数据库拷贝的测试环境（参见[第一章 安装Odoo开发环境](/blog/odoo-14-installing-odoo-development-environment)）。
