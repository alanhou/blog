...
description:
  en: "Configure email handling in Odoo 12 with mail threads, Jinja templates, scheduled mail actions, and activity management."
  zh: "配置Odoo 12中的邮件处理，包括邮件线程、Jinja模板、计划邮件动作和活动管理。"
date: 2019-08-11
tags: ["odoo", "email", "mail-thread", "jinja", "automation"]
...
本节我们将复用[第九章 高级服务端开发技巧](/blog/advanced-server-side-development-techniques)中的my_library模块。可以从本书GitHub仓库的Chapter09/r1_user_performing_actions目录中获取该模块的拷贝。本节中我们将对library.book.rent模型添加聊天器。
...
要求操作聊天器的字段和方法是mail.thread模型的一部分。mail.thread是一个抽象模型并且仅用于继承目的。在第2步中，我们在library.book.rent模型中继承了mail.thread模型。这会在library.book.rent模型中添加聊天器所需的必要字段和方法。如果你不知道模型继承如何运作，可参见[第五章 应用模型](/blog/application-models)中的*为可复用模型功能使用抽象模型*一节。
...
在第1步中，我们通过book_return_reminder_qweb ID创建了一个QWeb模板。如果你查看这个模板，我们不再使用 format_date()数据字段方法。这是因为QWeb渲染引擎自动根据用户的语言处理并展示日期。出于同样的原因，你不需要使用format_amount() 方法来展示货币符号。QWeb渲染引擎将会自动对其进行管理。如果想要了解更多有关QWeb模板的知识，可参见[第十五章 CMS网站开发](/blog/cms-website-development)中的*创建或更改模板 - QWeb*一节。
...
- 在Jinja中不能对其它模块修改一个已有模板，但在QWeb模板中你可以通过继承修改邮件模板。如果想要了解更多有关QWeb继承的知识，请参见[第十五章 CMS网站开发](/blog/cms-website-development)中的*创建或更改模板 - QWeb*一节。
