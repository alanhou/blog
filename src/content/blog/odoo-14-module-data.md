...
seriesOrder: 6
title:
  en: "Chapter 6: Managing Module Data"
  zh: "第6章 管理模块数据"
description:
  en: "Manage module data in Odoo 14 using XML and CSV data files, demo data, noupdate flags, and migration scripts."
  zh: "使用XML和CSV数据文件、演示数据、noupdate标志和迁移脚本管理Odoo 14中的模块数据。"
date: 2021-04-26
tags: ["odoo", "odoo14", "module-data", "xml", "csv", "migration"]
...
为避免重复输入大量代码，我们使用[第四章 应用模型](/blog/odoo-14-application-models)中所定义的模型。想按照下面的示例进行操作的话，请确保先从Chapter04/06_hierarchy_model中获取my_library模块的代码
...
2. 在<http://library.book/>模型中新建一本书：
...
参见[第一章 安装Odoo开发环境](/blog/odoo-14-installing-odoo-development-environment)来在 Odoo 中启动开发者模式。在激活了开发者模式之后，打开你希望查看XML ID的记录的表单视图。会在顶栏中看到一个调试图标。在该菜单中，点击View Metadata选项。参见如下截图：
...
此时若更新该模块，会看到我们所创建的出版社，并且如果像[第三章 创建Odoo插件模块](/blog/odoo-14-creating-odoo-add-on-modules)中所指出的那样启用了演示数据，还可以看到这本书以及其作者。
...
虽然基本上你可以对记录元素做任何操作，开发人员可以使用一些快捷元素来便捷地创建某种类型的记录。这包含菜单项、模板和动作窗口。参见[第九章 后端视图](/blog/odoo-14-backend-views)和[第十四章 CMS网站开发](/blog/odoo-14-cms-website-development)来获取更多相关信息。
...
这些语法与我们在[第五章 基本服务端开发](/blog/odoo-14-basic-server-side-development)*新建记录*和*更新记录集中记录值*小节中所讲解的相同。
...
最初，访问控制列表（ACL，参见[第十章 权限安全](/blog/odoo-14-access-security)）是一种通过CSV文件加载的数据。
...
在编写迁移脚本时，常常会碰到重复的任务，比如查某数据列或数据表是否存在、重命名或映射一些旧的值到新值。重复造轮子可能会容易产生问题并让人沮丧，如果可以接受额外依赖的话考虑使用<https://github.com/OCA/openupgradelib>。
