...
Chinchilla 论文挑战了"越大越好"的观点，发现当时的大模型（如 Gopher、GPT-3）都严重欠训练。
在相同计算预算下，更小但训练更充分的模型可以获得更好的性能。
...
Chinchilla 论文是 scaling laws 研究的重要修正，它表明在计算预算有限的情况下，应该平衡模型大小和训练数据量，而非一味追求更大的模型。

这一发现深刻影响了后续 LLM 的设计和训练策略。
