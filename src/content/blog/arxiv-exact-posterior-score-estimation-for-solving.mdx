...
description:
  ...
  zh: "\"本文在一般高斯插值框架下推导了线性高斯逆问题的精确后验分数闭式解，\\"
将后验采样转化为标准的去噪问题。\nEPS 方法在 FFHQ 和 ImageNet 上超越各类基线，\
同时仅需约十分之一的去噪器评估次数。"
date: 2026-06-16
...
They derive the exact posterior score \(\nabla`_{x_t}` \log p(x_t \mid y)\) in closed form:

\[
\nabla`_{x_t}` \log p(x_t \mid y) = \frac{1}{\sigma_t^2} \left( \text{some linear combination} \right)
\]
...
他们推导出后验分数 \(\nabla`_{x_t}` \log p(x_t \mid y)\) 的精确闭式：

\[
\nabla`_{x_t}` \log p(x_t \mid y) = \frac{1}{\sigma_t^2} \left( \text{某个线性组合} \right)
\]
