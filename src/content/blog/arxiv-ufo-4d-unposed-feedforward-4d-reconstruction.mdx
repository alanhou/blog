...
近期的前馈神经网络方法(如用于几何的DUSt3R或用于光流的RAFT)速度快但支离破碎:
一个网络估计静态几何,另一个估计二维运动,还有一个猜测相机位姿。
...
训练时,
所有头都从相同的高斯点渲染,
所以当你监督RGB输出(比较合成图像与真实图像)时,实际上也在间接教导深度和光流头——它们共享相同的几何基元。
...
其次,
transformer将这些细化为动态三维高斯点——每个高斯点获得均值位置$\mu$、
协方差$\Sigma$、颜色、不透明度,以及关键的速度向量。
