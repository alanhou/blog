...
Pass@k是可验证大语言模型任务的关键评估指标,广泛应用于数学推理、代码生成和简答题等场景,其定义为在$k$个独立采样的解决方案中至少有一个通过验证即为成功。
虽然这一指标激发了直接优化pass@k性能的推理感知微调方法,但一个持续存在的悖论浮现出来:pass@k的提升往往以pass@1性能下降为代价。
