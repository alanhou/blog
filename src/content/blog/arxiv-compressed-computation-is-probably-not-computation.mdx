...
模型的实际训练目标是\\(y`_{\text{mix}`} = M \cdot [\text{ReLU 输出}]\\)。
所以模型学的是输出50个近似这个混合信号的数——而不是原始的100个ReLU。

作者将训练目标分解为两部分：

\[
\mathcal{L} = \| \text{预测} - y`_{\text{mix}`} \|^2 = \| \text{预测} - M y`_{\text{clean}`} \|^2
\]
