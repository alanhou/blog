---
series: "odoo12-cookbook"
seriesOrder: 9
title:
  en: "Chapter 9: Advanced Server-Side Development Techniques"
  zh: "第9章 高级服务端开发技巧"
description:
  en: "Advanced Odoo 12 techniques including onchange methods, SQL queries, custom wizards, and the compute/inverse/search pattern."
  zh: "Odoo 12高级技巧，包括onchange方法、SQL查询、自定义向导以及compute/inverse/search模式。"
date: 2019-06-22
tags: ["odoo", "advanced-development", "wizards", "onchange", "sql"]
...
在[第六章 基本服务端部署](/blog/basic-server-side-development)中，我们学习了如何在模型类中编写方法、如何通过模型继承方法以及如何处理数据集。本章将讲解更为高级的话题，比如使用记录集的环境和操作onchange方法。
...
- 如果想要学习更多有关环境的知识，参见[第六章 基本服务端部署](/blog/basic-server-side-development)中*为不同模型获取一个空记录集*一节。
- 更多有关访问控制列表和记录规则的知识，请参见[第十一章 权限安全](/blog/access-security)。
...
- [第六章 基本服务端部署](/blog/basic-server-side-development)中*为不同模型获取一个空记录集*一节讲解了环境是什么。
- [第十章 后端视图](/blog/backend-views)中*向表单和动作传递参数 - 上下文*一节讲解了如何在动作定义中修改上下文。
- [第六章 基本服务端部署](/blog/basic-server-side-development)中*搜索记录*一节讲解了active记录。
...
本节中我们将使用前面小节的my_library模型。为进行简化，我们只会在日志中打印结果，但在真实场景中，你会需要在业务逻辑中使用查询结果。在[第十章 后端视图](/blog/backend-views)中，我们将在用户界面中展示查询的结果。
...
- 有关访问权限管理，请参见[第十一章 权限安全](/blog/access-security)。

## 编写向导来引导用户

在[第五章 应用模型](/blog/application-models)的*为可复用模型功能使用抽象模型*一节中，引用了models.TransientModel基类。这个类与常规的Model有很多的共通之处，不同点在于临时模型的记录会在数据库中定期地进行清除，因此称其为临时模型。它用于创建向导或对话框，来由用户在用户界面中填入并且通常用作执行对数据库中持久化记录的动作。
...
第3步为我们的向导定义了一个视图，参见[第十章 后端视图](/blog/backend-views)中*定义文档样式表单*一节获取更多详情。这里的重点是footer中的按钮，类型属性设置为了object，表示在用户点击该按钮时，会调用按钮name属性所指定的同名方法。

第4步中确保我们在应用的菜单中有一个向导的入口。我们在动作中使用了target='new' ，这样该表单视图会在当前表单上以对话框形式展示。参见[第十章 后端视图](/blog/backend-views)中的*添加菜单项和窗口动作*一节来了解详情。
...
但我们推荐使用这一部分中的代码，因为这让我们可以通过为向导创建记录复用代码中其它部分的向导，将它们放在单个记录集中（参见[第六章 基本服务端部署](/blog/basic-server-side-development)中的*合并记录集*一节了解如何实现），然后对记录集调用add_book_rents()。这里的代码很简单，无需跳过所有循环来记录某些书由不同成员借阅。但是在Odoo实例中，有些操作非常的复杂，带上可以实现正确操作的向导会非常的好。在使用这些向导时，确保你源代码中是否使用了上下文中的active_model/active_id/active_ids keys。如果使用了，需要传递一个自定义上下文（参见*使用变更的上下文调用方法*一节）。
...
在编写Odoo模型时，常常会有一些字段交叉关联。我们在[第五章 应用模型](/blog/application-models)中的*向模型添加约束验证*一节中学习过如何在字段中指定约束。本节讲解稍有不同的概念。这里在用户界面中一个字段被修改时调用onchange，来网页客户端中更新记录其它字段的值，通常是在表单视图中。
...
在[第六章 基本服务端部署](/blog/basic-server-side-development)中的*新建记录*和*更新记录集中记录值*小节中提到了这些操作不自动调用onchange方法。但是在一些用例中，调用这些操作又很重要，因为它们在所创建或所更新记录中更新重要的字段。当然， 你可以自己完成所需计算，但有时不可能做到，因为onchange可由你不知道的安装在实例中的第三方插件所添加或修改。
...
有关第1至3步的讲解，参见[第六章 基本服务端部署](/blog/basic-server-side-development)中*新建记录*一节。
...
如果想要了解创建和更新记录的更多知识，参见[第六章 基本服务端部署](/blog/basic-server-side-development)的*新建记录*和*更新记录集中记录值*小节。
...
4. 现在可以为新模型定义视图了。透视表视图对于挖掘数据尤为有用（参见[第十章 后端视图](/blog/backend-views)）。

5. 不要忘记为新模型定义一些权限规则（参见[第十一章 权限安全](/blog/access-security)）。
...
Odoo设置使用一个表单视图来在用户界面中展示settings选项。所有这些选项通过外部ID，base.res_config_settings_view_form在单个表单视图进行添加。第3步中，我们通过继承这一个设置表单视图来在用户界面中添加选项。[第十章 后端视图](/blog/backend-views)中，我们将进行更深入的学习。在表单定义中，我会发现这一选项的属性数据键值为模块名称。仅在向Settings添加在整个新的选项卡才会需要。否则，你只需通过xpath对已有模块的Settings选项卡中添加选项。
...
然后，你会看到一个该图书馆的单独Settings选项卡，其中，可心看到一个布尔字段用于启用或禁用自借阅选项。在启用或禁用这一选项时，背后Odoo会对所有odoo用户应用或删除implied_group。因为我们在按钮上添加了组，按钮会在用户拥有组时显示，而在用户没有组时隐藏。在[第十一章 权限安全](/blog/access-security)中，我们会深入地学习权限组。
...
在[第七章 模块数据](/blog/odoo12-module-data)中，我们学习了如何通过XML或CSV文件添加、更新及删除记录。但有时，业务用例非常复杂，无法通过使用数据文件来进行解决。这些情况下，可以在声明文件中使用init钩子来执行所需要的操作。
