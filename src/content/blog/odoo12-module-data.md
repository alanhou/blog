...
seriesOrder: 7
title:
  en: "Chapter 7: Module Data"
  zh: "第7章 模块数据"
description:
  en: "Manage module data in Odoo 12 using XML and CSV data files, demo data, and noupdate flags for configuration and reference data."
  zh: "使用XML和CSV数据文件、演示数据和noupdate标志管理Odoo 12中的模块数据。"
date: 2019-06-10
tags: ["odoo", "module-data", "xml", "csv", "demo-data"]
...
使用[第五章 应用模型](/blog/application-models)中的数据模型，我们将添加一本书和一位作者来作为演示数据。我们还会添加一个知名的出版社来作为模块中的常规数据。
...
此时若更新该模块，会看到我们所创建的出版社，并且如果像[第四章 创建Odoo插件模块](/blog/creating-odoo-add-on-modules)中所指出的那样启用了演示数据，还可以看到这本书以及其作者。
...
虽然基本上你可以对记录元素做任何操作，开发人员可以使用一些快捷元素来便捷地创建某种类型的记录。这包含menu item, template和 act window。参见[第十章 后端视图](/blog/backend-views)和[第十六章 网页客户端开发](/blog/web-client-development)来获取更多相关信息。
...
这些语法与我们在[第六章 基本服务端部署](/blog/basic-server-side-development)*新建记录*和*更新记录集中记录值*小节中所讲解的相同。
...
最初，访问控制列表（ACL，参见[第十一章 权限安全](/blog/access-security)）是通过CSV文件加载的一种数据类型。
