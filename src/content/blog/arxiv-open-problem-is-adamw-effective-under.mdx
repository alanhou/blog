...
\( \theta`_{t+1}` = \theta_t - \eta \frac{m_t}{\sqrt{v_t} + \epsilon} - \eta \lambda \theta_t \) (the weight decay term).  
...
参数更新为：  
\( \theta`_{t+1}` = \theta_t - \eta \frac{m_t}{\sqrt{v_t} + \epsilon} - \eta \lambda \theta_t \)（最后一项为权重衰减）。
