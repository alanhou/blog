...
通过在专门设计的带有明确推理约束的指令遵循数据集上微调模型,并引入使用独立LoRA适配器的解耦生成策略,他们在隐私和指令遵循基准测试中取得了显著改进。
在1.7B到14B参数的六个模型上测试,该方法在指令遵循方面实现了高达20.9分的提升,在隐私保护方面提升了51.9个百分点,尽管在任务效用方面存在一些权衡。
