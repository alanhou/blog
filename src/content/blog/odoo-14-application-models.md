...
seriesOrder: 4
title:
  en: "Chapter 4: Application Models"
  zh: "第4章 应用模型"
description:
  en: "Define and extend Odoo 14 models with various field types, computed fields, constraints, relationships, and inheritance mechanisms."
  zh: "定义和扩展Odoo 14模型，包括各种字段类型、计算字段、约束、关联关系和继承机制。"
date: 2021-04-26
tags: ["odoo", "odoo14", "models", "orm", "fields", "inheritance"]
image: "https://images.unsplash.com/photo-1517694712202-14dd9538aa97?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&q=80&w=1080"
---

:::zh
本章中的各小节会对已有的那个插件模型做一些小的新增。在上一单中，我们在Odoo实例中注册了自己的插件模块。本章中，我们将深入到模块的数据库端。会添加一个新模型(数据表)、一些新的字符和约束。我们还会研究Odoo中继承的使用。使用的是[第三章 创建Odoo插件模块](/blog/odoo-14-creating-odoo-add-on-modules)中所创建的模块。
...
要按照本章中的示例进行操作，你应该要有[第三章 创建Odoo插件模块](/blog/odoo-14-creating-odoo-add-on-modules)中所创建的模块并且该模型应可用。

本章中使用的代码可以在GitHub仓库中进行下载，地址为<https://github.com/alanhou/odoo14-cookbook/tree/main/Chapter04>。
...
这一节中我们假定你已经有一个my_library模块的实例，如[第三章 创建Odoo插件模块](/blog/odoo-14-creating-odoo-add-on-modules)中所描述。
...
第一步为定义的模型添加了对用户更友好的标题。这并非强制的，但可以为一些插件所用。例如，它可在创建记录时用于mail插件模块中追踪功能的通知文本。更多详情，请参见[第二十三章 在Odoo中管理Email](/blog/odoo-14-manage-emails-odoo)。如果在模型中不使用_description，Odoo会在日志中输出一条警告。
...
学习这一节假定读者已经有一个如[第三章 创建Odoo插件模块](/blog/odoo-14-creating-odoo-add-on-modules)中所述的带有my_library插件模块的实例准备就绪。
...
- groups让字段仅对安全组可用。它是包含安全组XML ID逗号分隔列表的一个字符串。这个话题将会在[第十章 权限安全](/blog/odoo-14-access-security)中进行讨论。
...
最后，我们根据模型中新增的字段更新了表单视图，我们可以在任意位置以自己的方式放置<field>标签。表单视图在[第九章 后端视图](/blog/odoo-14-backend-views)中会进行细致的讲解。
...
1. 通过Settings菜单下的链接启用开发者模式（参见[第一章 安装Odoo开发环境](/blog/odoo-14-installing-odoo-development-environment)中的*激活Odoo开发者工具*一节）。这会启用Settings > Technical菜单。
...
context和domain都将在[第十章 后端视图](/blog/odoo-14-access-security)中进行更详细的讲解。
...
> 📝要在用户界面中显示library.book.category模型，需要添加菜单、视图和权限规则。更多相关内容请参见[第三章 创建Odoo插件模块](/blog/odoo-14-creating-odoo-add-on-modules)。你也可通过访问[GitHub 仓库](https://github.com/alanhou/odoo14-cookbook)来获取代码。
...
我们将使用[第三章 创建Odoo插件模块](/blog/odoo-14-creating-odoo-add-on-modules)中所创建的图书模型，并向其添加一些约束。我们会添加一个数据库约束来防止重复的书名，以及一个 Python 模型约束来防止使用未来的日期作为发行日期。
...
我们在前面已经提到，也可以使用其它数据表约束。注意字段级约束如NOT NULL不能以这种方式进行使用。有关PostgreSQL的通用约束以及具体的数据表约束更详细的信息，请参见<http://www.postgresql.org/docs/current/static/ddl-constraints.html>。
...
> 📝本节会向已有模型新增字段。如果想在已有视图（用户界面）添加这些新字段的话，参见[第九章 后端视图](/blog/odoo-14-backend-views)中的*修改已有视图 - 视图继承*一节。
...
> 📝小贴士：要想查看新模型的菜单和视图，需要添加视图及菜单的XML定义。更多有关菜单和视图的相关内容，参见[第三章 创建Odoo插件模块](/blog/odoo-14-creating-odoo-add-on-modules)中的*添加菜单项和视图*一节。
...
models.TransientModel对于称之为向导的更为复杂的用户交互会非常有用。向导用于请求用户输入。在[第八章 高级服务端开发技巧](/blog/odoo-14-advanced-server-side-development-techniques)中，我们探讨如何使用它们来实现高级用户交互。
