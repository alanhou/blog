...
seriesOrder: 3
title:
  en: "Chapter 3: Creating Odoo Add-on Modules"
  zh: "第3章 创建Odoo插件模块"
description:
  en: "Create Odoo 14 addon modules from scratch, define models, add menu entries, set up access security, and use scaffolding."
  zh: "从零开始创建Odoo 14插件模块，定义模型、添加菜单项、设置访问权限并使用脚手架。"
date: 2021-04-26
tags: ["odoo", "odoo14", "modules", "addon-development", "scaffolding"]
...
本章中，我们将预设你已安装了Odoo并且按照[第一章 安装Odoo开发环境](/blog/odoo-14-installing-odoo-development-environment)中各节进行了操作。读者还应熟悉[第二章 管理Odoo服务端实例](/blog/odoo-14-managing-odoo-server-instances)中所描述的发现和安装额外插件模块的知识。
...
如果按照[第一章 安装Odoo开发环境](/blog/odoo-14-installing-odoo-development-environment)的*从源码轻松安装Odoo*一节进行操作的话，Odoo应该在~/odoo-dev/odoo下。为方便讲解，我们假定Odoo安装在该路径下，但是你可以使用其它你自己喜欢的路径。
...
- category：用于按照兴趣领域组织模块。标准的分类名称列表请见<https://github.com/odoo/odoo/blob/14.0/odoo/addons/base/data/ir_module_category_data.xml>。但也可以定义这些名称以外的新分类名称。
...
- data：这是在模块安装或升级时需加载数据文件的相对路径列表。这些路径相对于模块的根目录。通常，这些是XML和CSV文件，但也可以使用YAML格式的数据文件。这些内容会在[第六章 管理模块数据](/blog/odoo-14-module-data)中深入讨论。
...
- {pre_init, post_init, uninstall}_hook：这是在安装/卸载时调用的Python函数钩子。更多详细示例，请见[第八章 高级服务端开发技巧](/blog/odoo-14-advanced-server-side-development-techniques)。
...
- `models/`包含后端代码文件，用于创建模型及其业务逻辑。推荐每个模型一个文件并使用与模型相同的名称，例如 library.book模型的对应文件为library_book.py。这些在[第四章 应用模型](/blog/odoo-14-application-models)中会进行深入讲解。
- `views/`包含用于用户界面的XML文件，包含动作、表单、列表等等。类似于模型，建议每个模型一个文件。网站模板的文件名通常以_template后缀进行结尾。后端视图在[第九章 后端视图](9.md/)中讲解，网站视图在[第十四章 CMS网站开发](/blog/odoo-14-cms-website-development)中进行讲解。
- `data/`包含模块初始数据的其它数据文件。数据文件在[第六章 管理模块数据](/blog/odoo-14-module-data)中进行讲解。
- `demo/`包含带演示数据的数据文件，对于测试、培训或模块评测都非常有用。
- Odoo会在`i18n/`中查找.pot及.po翻译文件。更多详情参见[第十一章 国际化](/blog/odoo-14-internationalization)。这些文件无需在声明文件中声明。
- `security/`包含定义访问控制列表的数据文件，通常是一个ir.model.access.csv文件，也可以是一个XML文件，用于定义权限组及行级权限的记录规则。参见[第十章 权限安全](/blog/odoo-14-access-security)来获取更多内容。
- `controllers/`包含网站控制器的代码文件，用于为模块提供各种功能。网页控制器在[第十三章 Web服务端开发](/blog/odoo-14-manage-emails-odoo)中进行讲解。
- `static/`用于放置所有的网页资源。和其它目录不同，该目录名不只是一种惯例。这一目录中的文件无需用户登录即可对外提供访问。该目录多包含JavaScript、样式表、图像等文件。它们无需在模块声明文件中进行声明，但需要在网页模板中引用。这会在[第十四章 CMS网站开发](/blog/odoo-14-cms-website-development)中进行讨论。
- `wizard/` 包含所有与向导有关的文件。在Odoo中，向导用于存放中间数据。在[第八章 高级服务端开发技巧](/blog/odoo-14-advanced-server-side-development-techniques)中将会进一步学习到向导的
- `report/`: Odoo提供了为诸如销售订单和发票生成PDF文档的功能。该目录存放与PDF报告相关的所有文件。我们在[第十二章 自动化、工作流、Email和打印件](/blog/odoo-14-automation-workflows-printouts)中会进一步学习 PDF报告的相关知识。
...
有关模型有很多可以讨论的地方，我们将在[第四章 应用模型](/blog/odoo-14-application-models)中进行深度讲解。
...
虽然此处没有提供示例，业务逻辑也可以在这些Python文件中进行添加，或通过对模型类添加新的方法，或继承已有方法，如create() 或 write()。这在[第五章 基本服务端开发](/blog/odoo-14-basic-server-side-development)中进行讨论。
...
下一步是定义在客户端用户界面主区域中显示的窗口动作。该动作有一个由 res_model定义的目标模型，name属性用于在用户打开动作时向用户所显示的标题。这些都是基本属性。窗口动作还支持其它属性，让我们对视图渲染方式拥有更多的控制，比如显示什么视图，为可用的记录添加过滤器或设置默认值。这些会在[第九章 后端视图](/blog/odoo-14-backend-views)中进行讨论。
...
最后，我们添加了搜索视图来在右上角的搜索框中扩展搜索选项。在顶级的<search>标签中，可以包含<field>和<filter>元素。字段元素是在搜索视图中可用于搜索的其它字段。过滤器元素为可通过点击激活的预置过滤条件。这些话题会在[第九章 后端视图](/blog/odoo-14-backend-views)中进行讨论。
...
- 有关权限本书有单独的一章。更多权限相关的内容，参见[第十章 权限安全](/blog/odoo-14-access-security)。
