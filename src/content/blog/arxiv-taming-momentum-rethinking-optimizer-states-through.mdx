...
这些动量矩阵与模型参数同等规模:一个10亿参数模型用Adam训练时,
优化器状态本身就要占12GB显存(float32精度下存储参数、一阶动量、二阶动量三份拷贝)。
...
具体而言,
LoRA-Pre不直接存储动量$m_t \in \mathbb{R}^{d \times d}$,
而是分解为$m_t \approx A_t B_t^T$,
其中$A_t \in \mathbb{R}^{d \times r}$和$B_t \in \mathbb{R}^{d \times r}$且$r \ll d$。


作者证明当你将EMA视为求解$\min_{m} \sum_{i=1}^t \beta^{t-i} \|m - g_i\|^2$时,
这种分解自然涌现——这是一个加权最小二乘问题,
通过对$A$和$B$的在线梯度下降可得低秩解。
...
$$A_{t+1} = A_t - \eta abla_A \mathcal{L},
 \quad B_{t+1} = B_t - \eta abla_B \mathcal{L}$$
