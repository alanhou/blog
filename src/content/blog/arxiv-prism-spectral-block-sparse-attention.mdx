...
本文解决了长上下文LLM中块稀疏注意力机制的根本低效问题。
作者发现，均值池化与RoPE（旋转位置嵌入）的常见组合通过过滤高频信号为局部位置信息创建了"盲点"。
Prism通过使用基于能量的温度校准将块选择分离为高频和低频分量来解决这个问题。
该方法无需训练，在实现与全注意力精度持平的同时提供最高5.1倍的加速。
...
$$\text{RoPE}(x,
 m) = x \odot \cos(m\theta) + \text{rotate}(x) \odot \sin(m\theta)$$
...
Prism优雅地解决了高效长上下文注意力中的一个根本问题。
通过识别均值池化在RoPE表示上创建的频谱盲点并提供有原则的分解解决方案，它实现了两全其美：块稀疏注意力的效率与全注意力的精度。
无需训练的特性使其可以立即部署，信号处理中的理论基础提供了对其工作原理的清晰理解。
