...
seriesOrder: 5
title:
  en: "Chapter 5: Application Models"
  zh: "第5章 应用模型"
description:
  en: "Define and extend Odoo 12 models with various field types, computed fields, constraints, relationships, and inheritance mechanisms."
  zh: "定义和扩展Odoo 12模型，包括各种字段类型、计算字段、约束、关联关系和继承机制。"
date: 2019-05-23
tags: ["odoo", "models", "orm", "fields", "inheritance"]
...
本章中的各小节会对已有的那个插件模型做一些小的新增。我们将使用在[第四章 创建Odoo插件模块](/blog/creating-odoo-add-on-modules)中所创建的模块。
...
第一步为定义的模型添加了一个用户友好的标题。这并非强制的，但可以为一些插件所用。例如，它可在创建记录时用于mail插件模块中追踪功能的通知文本。更多详情，请参见[第二十三章 在Odoo中管理email](/blog/manage-emails-odoo)。
...
这一节假定你已经有一个如[第四章 创建Odoo插件模块](/blog/creating-odoo-add-on-modules)中所述的带有my_library插件模型的实例准备就绪。
...
- groups让字段仅对安全组可用。它是包含安全组的XML ID逗号分隔列表的一个字符串。这个话题将会在[第十一章 权限安全](/blog/access-security)中进行讨论。
...
最后，我们根据模型中新增的字段更新了表单视图，我们在这里以自己的方式放置<field>标签，但你可以根据自己的需要放置在任意位置。表单视图在[第十章 后端视图](/blog/backend-views)中会进行细致的讲解。
...
我们将复用在[第四章 创建Odoo插件模块](/blog/creating-odoo-add-on-modules)中创建的my_library插件模块。
...
2. 通过Settings菜单下的链接激活开发者模式（参见[第一章 安装Odoo开发环境](/blog/installing-odoo-development-environment)中的*激活Odoo开发者工具*一节）。这会启用Settings > Technical菜单。
...
我们将复用[第四章 创建Odoo插件模块](/blog/creating-odoo-add-on-modules)中的my_library插件模块。
...
context和domain都将在[第十章 后端视图](/blog/backend-views)中进行更详细的讲解。
...
> ℹ️要在用户界面中显示library.book.category模型，你需要添加菜单、视图和权限规则。更多相关内容请参见[第四章 创建Odoo插件模块](/blog/creating-odoo-add-on-modules)。你也可通过访问[GitHub 仓库](https://github.com/alanhou/odoo12-cookbook)来获取代码。
...
我们将使用[第四章 创建Odoo插件模块](/blog/creating-odoo-add-on-modules)中所创建的图书模型，并向其添加一些约束。我们会添加一个数据库约束来防止重复的书名，以及一个 Python 模型约束来防止使用未来的日期作为发行日期。

### 准备工作

本节中，我们将在library.book模型中添加约束。为此我们使用[第四章 创建Odoo插件模块](/blog/creating-odoo-add-on-modules)中的my_library模型。
...
> ℹ️本节会向已有模型新增字段。如果你想在已有视图（用户界面）添加这些新字段的话，参见[第十章 后端视图](/blog/backend-views)中的*修改已有视图 - 视图继承*一节。
...
models.TransientModel对于称之为向导的更为复杂的用户交互会更为有用。该向导用于从用户请求输入。在[第九章 高级服务端开发技巧](/blog/advanced-server-side-development-techniques)中，我们探讨如何使用它们来实现高级用户交互。
