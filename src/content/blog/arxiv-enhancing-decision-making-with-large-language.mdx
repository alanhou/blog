...
Data flow:  
- At round \(t\), each agent \(i\) computes the mixture \(\pi`_{-i}`^t\) = average of all actions in the histories of all other agents.  
- Then \(i\) prompts the LLM with a description of its own utility and \(\pi`_{-i}`^t\) to get a best-response action \(a_i^t\).  
...
数据流动：  
- 在轮次\(t\)，每个智能体\(i\)计算混合策略\(\pi`_{-i}`^t\) = 所有其他智能体历史记录中所有动作的平均。  
- 然后\(i\)向LLM输入自身效用描述和\(\pi`_{-i}`^t\)，得到最佳响应动作\(a_i^t\)。  
