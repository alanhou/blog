...
seriesOrder: 8
title:
  en: "Chapter 8: Advanced Server-Side Development Techniques"
  zh: "第8章 高级服务端开发技巧"
description:
  en: "Advanced Odoo 14 techniques including onchange methods, SQL queries, custom wizards, install hooks, and the compute/inverse/search pattern."
  zh: "Odoo 14高级技巧，包括onchange方法、SQL查询、自定义向导、安装钩子以及compute/inverse/search模式。"
date: 2021-04-26
tags: ["odoo", "odoo14", "advanced-development", "wizards", "onchange", "sql"]
image: "https://images.unsplash.com/photo-1517694712202-14dd9538aa97?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&q=80&w=1080"
---

:::zh
在[第五章 基本服务端开发](/blog/odoo-14-basic-server-side-development)中，我们学习了如何在模型类中编写方法、如何扩展所继承模型的方法以及如何处理记录集。本章将讲解更为高级的话题，比如使用记录集的环境、对按钮点击调用方法和操作onchange方法。本章中的各小节有助于处理更复杂的业务问题。我们将学习如何创建交互性更强的应用。
...
需要添加一个表单视图、一个动作以及一个通过用户界面查看新模型的菜单项。还需要为librarian添加一个权限规则，这样他们可以发布供出借的图书。如果不了解如保添加，请参见[第三章 创建Odoo插件模块](/blog/odoo-14-creating-odoo-add-on-modules)。

同时，也可以使用GitHub上已经编写好的初始模块代码示例来节约一点时间。该模块位于Chapter08/00_initial_module文件夹下。GitHub 代码示例请参见<https://github.com/alanhou/odoo14-cookbook/tree/main/Chapter08/00_initial_module>。
...
- 如果想要学习更多有关环境的知识，参见[第五章 基本服务端开发](/blog/odoo-14-basic-server-side-development)中*为不同模型获取一个空记录集*一节。
- 更多有关访问控制列表和记录规则的知识，请参见[第十章 权限安全](/blog/odoo-14-access-security)。
...
- [第五章 基本服务端开发](/blog/odoo-14-basic-server-side-development)中*为不同模型获取一个空记录集*一节讲解了环境是什么。
- [第九章 后端视图](/blog/odoo-14-backend-views)中*向表单和动作传递参数 - 上下文*一节讲解了如何在动作定义中修改上下文。
- [第五章 基本服务端开发](/blog/odoo-14-basic-server-side-development)中*搜索记录*一节讲解了active记录。
...
本节中我们将使用前面小节的my_library模块。为进行简化，我们只会在日志中打印结果，但在真实场景中，会需要在业务逻辑中使用查询结果。在[第九章 后端视图](/blog/odoo-14-backend-views)中，我们将在用户界面中展示查询的结果。
...
- 有关访问权限管理，请参见[第十章 权限安全](/blog/odoo-14-access-security)。

## 编写向导来引导用户

在[第四章 应用模型](/blog/odoo-14-application-models)的*为可复用模型功能使用抽象模型*一节中，介绍了models.TransientModel基类。这个类与常规的Model有很多的共通之处，不同点在于临时模型的记录会在数据库中定期地进行清除，因此称其为临时模型。它用于创建向导或对话框，来由用户在用户界面中填入并且通常用作执行对数据库中持久化记录的动作。
...
第3步为我们的向导定义了一个视图，参见[第九章 后端视图](/blog/odoo-14-backend-views)中*定义文档样式表单*一节获取更多详情。这里的重点是footer中的按钮，type属性设置为了object，表示在用户点击该按钮时，会调用按钮name属性所指定的同名方法。

第4步保障我们在应用的菜单中有一个向导的入口。我们在动作中使用了target='new' ，这样该表单视图会在当前表单之上以对话框形式展示。参见[第九章 后端视图](/blog/odoo-14-backend-views)中的*添加菜单项和窗口动作*一节来了解详情。
...
我们推荐使用这一小节中的代码，因为这让我们可以通过对向导创建记录复用代码中其它部分的向导，将它们放在单个记录集中（参见[第五章 基本服务端开发](/blog/odoo-14-basic-server-side-development)中的*合并记录集*一节了解如何实现），然后对记录集调用add_book_rents()。这里的代码很简单，无需执行完所有循环来记录某些书由不同成员借阅。但是在Odoo实例中，有些操作非常的复杂，带有可以实现正确操作的向导会非常的好。在使用这些向导时，务必检查源代码中是否使用了上下文中的active_model/active_id/active_ids keys。如果使用了，需要传递一个自定义上下文（参见*使用变更的上下文调用方法*一节）。
...
- 参见[第九章 后端视图](/blog/odoo-14-backend-views)中*定义文档样式表单*一节获取更多对向导定义视图的详情
- 参见[第九章 后端视图](/blog/odoo-14-backend-views)中的*添加菜单项和窗口动作*一节了解有关视图和调用服务端方法的知识
- 参见[第五章 基本服务端开发](/blog/odoo-14-basic-server-side-development)中的*合并记录集*一节了解为向导创建记录和将它们放入单个记录集的详情

## 定义onchange方法

在编写业务逻辑时，常常会有一些字段交叉关联。我们在[第四章 应用模型](/blog/odoo-14-application-models)中的*向模型添加约束验证*一节中学习过如何在字段中指定约束。本节讲解稍有不同的概念。这里在用户界面中一个字段被修改时调用onchange，来在网页客户端中更新记录其它字段的值，这通常是在表单视图中。
...
有关第1至3步的讲解，参见[第五章 基本服务端开发](/blog/odoo-14-basic-server-side-development)中*新建记录*一节。
...
如果想要了解创建和更新记录的更多知识，参见[第五章 基本服务端开发](/blog/odoo-14-basic-server-side-development)的*新建记录*和*更新记录集中记录值*小节。
...
参见[第四章 应用模型](/blog/odoo-14-application-models)中的*向模型添加计算字段*一节学习更多有关计算字段的知识。
...
4. 现在可以为新模型定义视图了。透视表视图对于挖掘数据尤为有用（参见[第九章 后端视图](/blog/odoo-14-backend-views)）。

5. 不要忘记为新模型定义一些权限规则（参见[第十章 权限安全](/blog/odoo-14-access-security)）。
...
- 参见[第九章 后端视图](/blog/odoo-14-backend-views)了解更多有关UI视图的知识。

- 参见[第十章 权限安全](/blog/odoo-14-access-security)进一步了解权限控制和记录规则。
...
然后，会看到一个该图书馆的单独Settings选项卡，其中，可以看到一个布尔字段用于启用或禁用自借阅选项。在启用或禁用这一选项时，背后Odoo会对所有odoo用户应用或删除implied_group。因为我们在按钮上添加了组，按钮会在用户拥有组时显示，而在用户没有组时隐藏。在[第十章 权限安全](/blog/odoo-14-access-security)中，我们会深入地学习权限组。
...
在[第六章 管理模块数据](/blog/odoo-14-module-data)中，我们学习了如何通过XML或CSV文件添加、更新及删除记录。但有时，业务用例非常复杂，无法通过数据文件进行解决。这类情况下，可以在声明文件中使用init钩子来执行所需要的操作。
