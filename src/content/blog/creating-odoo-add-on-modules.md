...
seriesOrder: 4
title:
  en: "Chapter 4: Creating Odoo Add-on Modules"
  zh: "第4章 创建Odoo插件模块"
description:
  en: "Create Odoo 12 addon modules from scratch, define models, add menu entries, set up access security, and use scaffolding."
  zh: "从零开始创建Odoo 12插件模块，定义模型、添加菜单项、设置访问权限并使用脚手架。"
date: 2019-05-18
tags: ["odoo", "modules", "addon-development", "scaffolding"]
...
如果我按照[第一章 安装Odoo开发环境](/blog/installing-odoo-development-environment)的*从源码轻松安装Odoo*一节进行操作的话，Odoo应该在~/odoo-dev/odoo下。为方便讲解，我们假定Odoo安装在该路径下，但是你可以使用其它你自己偏好的路径。
...
- {pre_init, post_init, uninstall}_hook：这是在安装/卸载时调用的Python函数钩子。更多详细示例，请见[第九章 高级服务端开发技巧](/blog/advanced-server-side-development-techniques)。
...
有关模型有很多可以讨论的地方，我们将在[第五章 应用模型](/blog/application-models)中进行深度讲解。
...
虽然此处没有提供示例，业务逻辑也可以在这些Python文件中进行添加，或通过对模型类添加新的方法，或继承已有方法，如create() 或 write()。这在[第六章 基本服务端部署](/blog/basic-server-side-development)中进行讨论。
...
下一步是定义在客户端主区域用户界面中显示的窗口动作。该动作有一个由 res_model,定义的目标模型，name属性用于在用户打开动作时向用户所显示的标题。这些都是基本属性。窗口动作还支持其它属性，让我们对视图渲染方式拥有更多的控制，比如显示什么视图，为可用的记录添加过滤器或设置默认值。这些会在[第十章 后端视图](/blog/backend-views)中进行讨论。
...
最后，我们添加了搜索视图来在右上角的搜索框中扩展搜索选项。在顶级的<search>标签中，可以包含<field>和<filter>元素。字段元素是在搜索视图中可用于搜索的其它字段。过滤器元素为可通过点击激活的预置过滤条件。这些话题会在[第十章 后端视图](/blog/backend-views)中进行讨论。
...
- 有关权限本书有单独的一章。更多有关权限的内容，参见[第十一章 权限安全](/blog/access-security)。
