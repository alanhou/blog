...
- **队列约简**：对齐范数之后，向量值状态（每个产品的库存水平）坍缩为一个单标量：从目标到可行集的距离。动力学变为 \( d`_{t+1}` = \max(d_t + \Delta_t - D_t, 0) \)，其中 \(\Delta_t\) 是目标漂移（由学习者控制），\(D_t\) 是共性需求（随机）。这是经典的离散时间队列；学习者遗憾可用排队论参数（到达率对服务率）来界。这一约简正是分析能扩展到**任意**凸容量集的原因，而此前工作只能逐坐标处理。
