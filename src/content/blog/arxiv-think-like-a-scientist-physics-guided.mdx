...
image: "/arxiv-visuals/arxiv-think-like-a-scientist-physics-guided.png"
---
...
从观测现象中发现符号方程是科学研究的核心目标。
虽然大语言模型因其推理能力在方程发现方面展现出潜力,但现有方法试图直接从数据猜测方程,而没有遵循科学家使用的结构化推理过程。
本文提出KeplerAgent,一个明确建模科学推理的智能体框架:它首先从数据中推断对称性和守恒律等物理属性,然后将这些洞察作为结构先验来约束符号回归。
通过协调基于物理的工具与PySINDy和PySR等符号回归引擎,KeplerAgent在符号精度和噪声鲁棒性方面均优于纯LLM方法和传统方法。
