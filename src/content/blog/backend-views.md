...
seriesOrder: 10
title:
  en: "Chapter 10: Backend Views"
  zh: "第10章 后端视图"
description:
  en: "Create and customize Odoo 12 backend views including form, tree, search, kanban, calendar, graph, pivot, and activity views."
  zh: "创建和自定义Odoo 12后端视图，包括表单、列表、搜索、看板、日历、图表、透视表和活动视图。"
date: 2019-07-06
tags: ["odoo", "views", "backend", "form", "kanban", "tree"]
...
贯穿本章，我们将假定你的数据库中安装了base插件并且有一个空的插件模块可以从各节中添加XML代码到插件声明文件所引用的数据文件中。参见[第四章 创建Odoo插件模块](/blog/creating-odoo-add-on-modules)，获取更多有关激活插件中修改的信息。
...
你一定注意到了分类字段的widget属性。它定义了字段中的数据向用户展示的方式。每种类型的字段都有一个标准组件，因此无需显式的指定widget。但有些类型提供了多种展现方式，这时你可能会选择非默认的方式。可用组件的完整列表超出了本节的范畴，请参照Odoo源代码来进行尝试。参见[第十六章 网页客户端开发](/blog/web-client-development)来了解如何制作自己的组件的相关知识。
...
- 参见[第十六章 网页客户端开发](/blog/web-client-development)中*创建自定义组件*一节来定义你自己的组件。
- 参见[第十六章 网页客户端开发](/blog/web-client-development)中*新建一个视图*一节来创建你自己的视图。
...
对于传统字段这些都没有问题，但在搜索非存储函数字段时这是一个广为人知的问题。通过[第五章 应用模型](/blog/application-models)中所讲到的在代码中加入搜索函数即可进行处理。
