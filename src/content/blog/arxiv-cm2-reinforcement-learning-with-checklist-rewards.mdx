...
date: 2026-02-13
tags: ["arxiv", "ai", "cs.ai"]
image: "/arxiv-visuals/arxiv-cm2-reinforcement-learning-with-checklist-rewards.png"
---
...
然而,
在这类场景中应用强化学习面临根本性挑战:现实目标往往缺乏可验证的奖励,
多轮多步骤智能体工具使用在强化学习语境下仍未得到充分探索,且构建可执行工具环境的成本高昂。


本文提出CM2(多轮多步骤工具使用的检查清单奖励),这是一个用结构化检查清单奖励替代传统可验证结果奖励的强化学习框架。
通过将预期行为分解为具有明确证据支撑的细粒度二元标准,CM2将开放式评估转化为稳定的分类决策。
该框架采用稀疏奖励分配与密集评估标准相结合的策略,并在可扩展的大语言模型模拟环境中训练。
实验结果表明相比监督微调有持续改进,
8B模型在$\tau$-Bench上获得8分提升,
在BFCL-V4上提升10分,
在ToolSandbox上提升12分,达到或超越同等规模的开源基线模型。
...
$$R(s,
 a) = \sum_{i=1}^{n} w_i \cdot \mathbb{1}[\text{criterion}_i(s, a) = \text{true}]$$
