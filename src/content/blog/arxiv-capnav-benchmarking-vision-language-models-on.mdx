...
该基准涵盖45个真实室内场景、473个导航任务和2365个问答对,涉及五种不同的智能体类型——从轮椅使用者到四足机器人——每种都具有独特的物理尺寸和移动能力。
对13个最先进视觉语言模型的评估揭示了一个关键局限:随着移动约束变得更加严格,导航性能显著下降,模型在需要空间维度推理的障碍物上表现尤其困难。
...
评估框架包含两个主要任务:(1)导航可行性评估,模型必须判断给定路径对特定智能体是否可通行;(2)障碍物识别和推理,模型必须识别哪些障碍物阻止导航并解释原因。
...
第三,错误分析揭示模型经常犯两类错误:(1)过度泛化,假设所有智能体具有相似能力;(2)维度盲区,无法推理智能体的物理尺寸是否允许通过某个空间。
