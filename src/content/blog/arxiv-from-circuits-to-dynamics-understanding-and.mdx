...
当模型正常生成时,奇异值遵循特征性分布。
然而当崩溃发生时,谱熵 $H = -\sum_i \lambda_i \log \lambda_i$(其中$\lambda_i$为归一化奇异值)显著增加,表明信息在奇异向量间的分布更加均匀。
...
该方法仅在测试时应用,无需模型重训练。
在WaLa和Make-a-Shape架构以及GSO和SimJEB数据集的实验中,PowerRemap实现了87.4%至98.3%的稳定化率,显著减少碎片化事件。
