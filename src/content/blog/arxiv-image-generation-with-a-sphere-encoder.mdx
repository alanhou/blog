...
本文提出了球面编码器(Sphere Encoder),这是一种高效的生成模型,通过单次前向传播或少于五步的迭代即可实现与扩散模型相媲美的图像质量,挑战了迭代扩散模型的主导地位。
...
球面编码器框架由两个主要组件构成:编码器 $E$ 将图像 $x$ 映射到 $d$ 维空间中的单位超球面 $\mathcal{S}^{d-1}$ 上的点,解码器 $D$ 从这些球面表示重建图像。
关键洞察在于,通过强制编码图像在球面上均匀分布,随机采样等价于从数据分布中采样。


训练目标专注于重建质量:$\mathcal{L} = \mathbb{E}_{x \sim p_{data}}[\|x - D(E(x))\|^2]$,其中编码器输出被归一化以位于单位球面上。
这种简单的表述避免了对抗损失或变分界的复杂性。
通过架构选择和归一化来鼓励球面上的均匀分布,确保解码器学习将整个球面映射到有效图像。


对于生成过程,模型只需从球面上的均匀分布采样一个随机点 $z \sim \mathcal{U}(\mathcal{S}^{d-1})$,然后计算 $\hat{x} = D(z)$。
这种单步过程与需要迭代去噪的扩散模型形成鲜明对比。
可选地,可以通过应用编码器-解码器循环来提升质量:$\hat{x}_t = D(E(\hat{x}_{t-1}))$ 进行几次迭代,这起到了优化机制的作用。
