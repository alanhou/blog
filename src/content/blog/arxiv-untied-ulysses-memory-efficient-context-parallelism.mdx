...
通过将注意力计算分解到各个注意力头并分布到加速器上,UPipe在320亿参数模型中实现了高达87.5%的中间张量内存减少,同时保持与Ring Attention和DeepSpeed Ulysses等现有方法相当的训练吞吐量。
...
数学表述显示,对于具有$H$个注意力头和序列长度$L$的模型,传统方法每个设备需要$O(L \cdot H \cdot d)$的激活内存,其中$d$是头维度。
