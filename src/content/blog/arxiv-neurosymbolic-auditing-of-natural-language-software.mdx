...
具体例子：需求说"压力 `> 1`00 时报警激活"，另一条说"压力 ≤ 150 时报警永不激活"。

SMT 求解器发现这些是一致的（两者都可为真），但如果第二条说"压力 `> 5`0 时报警永不激活"，SMT 会标记矛盾。
...
例子：形式化说"压力 `> 1`00 时报警触发"，但测试案例显示报警应在压力 = 100 时触发（边界情况）。
