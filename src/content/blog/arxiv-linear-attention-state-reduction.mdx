...
```
rank(S_t) ≤ min(rank(K_t), rank(V_t)) ≤ t
```
...
```
u(S) = sr(S) / min(d_v, d_k)
```

- **Low utilization (u ≪ 1)**: Rank collapse, energy concentrated in few components, redundant information
- **High utilization (u ≈ 1)**: Isotropic memory, energy evenly distributed

### Impact on Retrieval Error

For a noisy query q̃ = q* + n:
- Low rank utilization **amplifies query noise**
- High rank utilization provides more robust retrieval

Key insight: The conditioning of keys κ(K_t) is tightly connected to the effective rank of the hidden state.
...
- **低利用率 (u ≪ 1)**：秩崩溃，能量集中在少数分量，信息冗余
- **高利用率 (u ≈ 1)**：各向同性记忆，能量均匀分布
...
关键洞察：键的条件数 κ(K_t) 与隐藏状态的有效秩紧密相关。
