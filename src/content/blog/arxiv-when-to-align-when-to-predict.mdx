...
description:
  ...
  zh: "\"这篇论文系统分析了跨模态对齐与跨模态预测何时有效。"
引入相位图帮助在多模态学习前诊断问题并选择正确目标。"
date: 2026-06-10
tags: ["arxiv", "ai", "cs.lg"]
...
- **CA (cross-modal alignment)**: minimize the distance between the representations of corresponding pairs. This whitens each modality (removes its covariance structure) and forces the representations to match. They derive a **CA separation ratio** \(R`_{CA}`\) that measures how much signal survives after whitening versus how much nuisance leaks through.
- **CP (cross-modal prediction)**: predict the representation of one view from the other, using a one-sided whitening (whiten only the source modality). They derive a **CP separation ratio** \(R`_{CP}`\) that depends on the source modality's quality.
...
The ratios define a two-dimensional coordinate. Plotting \( (R`_{CA}`, R`_{CP}`) \) partitions the plane into four regimes: Both, CA only, CP only, Neither. A data-driven procedure estimates these ratios from a small labeled subsample (e.g., 50-100 pairs) by decomposing empirical covariances.
...
- **Phase diagram**: A 2D map with \(R`_{CA}`\) on x-axis and \(R`_{CP}`\) on y-axis, divided into four quadrants by the lines \(R`_{CA}`=1\) and \(R`_{CP}`=1\). Each quadrant corresponds to a regime where at least one of the objectives works. The origin (both &lt;1) is "Neither" – cross-modal training is actively worse than using a single modality.
...
2. **The "Neither" regime is real**: If both \(R`_{CA}`&lt;1\) and \(R`_{CP}`&lt;1\), cross-modal training will actively degrade performance. The astrophysical example shows this can happen in practice. Practitioners should check this before committing resources.
...
- **CA（跨模态对齐）**：最小化对应样本表示之间的距离。
  ...
  他们推导出**CA分离比** \( R`_{CA}` \)，衡量白化后有多少信号留存，以及有多少干扰泄漏。
- **CP（跨模态预测）**：用一张视图预测另一张视图的表示。
  这使用单侧白化（只白化源模态）。
  他们推导出**CP分离比** \( R`_{CP}` \)，依赖于源模态的质量。
...
这两个比值定义了一个二维坐标。
将 \( (R`_{CA}`, R`_{CP}`) \) 绘制在平面上，划分为四个区域：都行、仅CA、仅CP、都不行。
...
- **相位图**：以 \(R`_{CA}`\) 为x轴、\(R`_{CP}`\) 为y轴的二维图，
  由 \(R`_{CA}`=1\) 和 \(R`_{CP}`=1\) 两条线划分为四个象限。
...
1. **诊断流程**：在任何跨模态训练之前，收集几百个标注对（若无标注，有少量监督信号也可），
   ...
   无需训练完整网络即可获得可信的选择。
2. **"都不行"区域是真实的**：如果 \(R`_{CA}`&lt;1\) 且 \(R`_{CP}`&lt;1\)，跨模态训练会主动降低性能。
