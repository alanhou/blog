...
伊藤映射是一个函数 \( f： (x_t, B`_{[t,T]}`) \mapsto x_T \)，
其中 \(x_t\) 是中间状态，\(B`_{[t,T]}`\) 是从时间 \(t\) 到 \(T\) 的完整布朗路径。
...
然后训练神经网络 \(f_\theta(x_t, B`_{[t,T]}`)\) 预测 \(x_T\)，使用简单的MSE损失。
