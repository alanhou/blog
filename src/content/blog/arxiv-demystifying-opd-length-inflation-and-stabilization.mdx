...
The compression-based repetition metric (CompRatio `> 1`0 on tail tokens) is a simple, practical way to monitor training health. Add it to your logging alongside truncation rate.
...
基于压缩的重复指标（尾部 token 上 CompRatio `> 1`0）是监控训练健康的简单实用方法。将其与截断率一起添加到日志中。
