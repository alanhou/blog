...
这种方法在TriviaQA上获得9.9%的提升,并很好地迁移到其他基准测试:Natural Questions(+4.2%)、HotpotQA(+2.1%)、SimpleQA(+0.6%)和StrategyQA(+3.0%)。
...
更重要的是,这些改进迁移到了未见过的数据集:Natural Questions显示4.2%的增长,HotpotQA提高2.1%,StrategyQA获得3.0%的提升。
...
在一个知识密集型数据集(TriviaQA)上的训练改善了其他数据集的性能,表明模型学习的是导航其参数化知识的广泛适用策略,而不是数据集特定的技巧。
