...
在三个生物医学数据集上的评估显示,FGNO取得了显著改进:神经信号解码中AUROC提升35%,温度预测中RMSE降低16%,以及在低数据条件下睡眠阶段分类准确率提高超过20%。
...
具有低损坏($t \approx 0$)的早期层保留细粒度时间模式,而具有较高损坏($t \approx 1$)的深层捕获全局结构和长程依赖关系。
