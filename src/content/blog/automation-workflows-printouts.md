...
seriesOrder: 13
title:
  en: "Chapter 13: Automation, Workflows, and Printouts"
  zh: "第13章 自动化、工作流和打印件"
description:
  en: "Automate Odoo 12 processes with server actions, scheduled tasks, kanban workflows, QWeb reports, and custom printout templates."
  zh: "使用服务器动作、计划任务、看板工作流、QWeb报表和自定义打印模板自动化Odoo 12流程。"
date: 2019-07-19
tags: ["odoo", "automation", "workflows", "qweb", "reports"]
...
第6章中，我们添加了不同阶段的默认值。用户在安装这个模块后会年到这些基本阶段。如果想要学习XML数据语法的更多知识，请参见[第七章 模块数据](/blog/odoo12-module-data)中的*使用XML文件加载数据*一节。
...
在第1步中，我们为 library.book.rent模型添加了一个看板视图。注意我们使用了stage_id作为看板的默认组，因此在用户打开看板时，看板卡片会按阶段进行分组。了解更多有关看板的知识，请参见[第十章 后端视图](/blog/backend-views)。
...
我们所使用的代码计算当前日期到截止日期的天数，并使用它来准备相应的通知消息。最后一行在任务的消息墙中进行了实际的消息发布。邮件通知需要subtype='mt_comment'来将其发送给订阅者，和我们使用New Message按钮的情况一样。如果未给定子类型，默认使用mt_note，发布内部记录而不进行通过，和使用 Log an internal note按钮的效果一样。参见[第二十三章 在Odoo中管理email](/blog/manage-emails-odoo)来学习Odoo中有关mailing更多的知识。
...
两种方法可使用过滤器来过滤出仅用于执行动作的记录。我们可以使用域表达式来进行实现。可以在[第十章 后端视图](/blog/backend-views)中找到有关编写域表达式的更进一步信息。此外，我们可以使用用户界面功能对项目任务创建并保存过滤器，然后复制自动生成的域表达式，从Set selection based on a search filter列表中进行选择。
...
在[第六章 基本服务端部署](/blog/basic-server-side-development)中，我们学习了如何重写义模型的write()方法来对记录更新执行动作。记录更新自动化动作提供另一种实现方式，有诸多好处和缺点。
...
我尚未安装wkhtmltopdf，请按照[第一章 安装Odoo开发环境](/blog/installing-odoo-development-environment)中所描述的方式进行安装，否则将无法得到漂亮的PDF文件。
...
现在不必担心template元素中的语法。这个话题会在[第十六章 网页客户端开发](/blog/web-client-development)中的*使用客户端QWeb模板*一节是进行深入讲解。
