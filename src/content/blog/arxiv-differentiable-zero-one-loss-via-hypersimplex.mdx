...
The order-preserving property ensures that if logit $z_i `> z`_j$, then the projected value $x_i \geq x_j$, maintaining the ranking structure essential for classification tasks.
...
保序性质确保如果logit $z_i `> z`_j$,则投影值$x_i \geq x_j$,保持了分类任务所需的排序结构。
