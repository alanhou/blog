...
seriesOrder: 24
title:
  en: "Chapter 24: IoT Box"
  zh: "第24章 IoT盒子"
description:
  en: "Set up and configure the Odoo 12 IoT Box with Raspberry Pi for connecting printers, scales, and other peripheral devices."
  zh: "使用树莓派设置和配置Odoo 12 IoT盒子，连接打印机、秤和其他外围设备。"
date: 2019-07-13
tags: ["odoo", "iot", "raspberry-pi", "hardware", "peripherals"]
...
我们将使用[第二十三章 在Odoo中管理email](/blog/manage-emails-odoo)中*在聊天器中记录用户修改*一节的my_library模块。本节中，我们将新增一个在借阅者归还图书时捕获和存储图像的字段。确保打开了IoT盒子并且通过它连接了可支持的摄像头设备。
...
在第1步中，我们在声明文件中添加了对quality_mrp_iot模块的依赖。quality_mrp_iot模块属于企业版，包含通过IoT盒子来使用摄像头请求图像的组伯。这会安装mrp模块，但为进行简化，我们将使用quality_mrp_iot来作为依赖。如果你不想要使用这个依赖，可以创建自己的字段组件。参见[第十六章 网页客户端开发](/blog/web-client-development)来学习有关组件的更多知识。
