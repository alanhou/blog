...
与使用手工设计的后期融合策略的现有方法不同,该方法通过学习式的逐查询融合实现更深层的跨模态协同,在保持开放词汇能力的同时,显著缩小了零样本与全监督分割之间的性能差距。
...
在标准语义分割数据集上,仅使用5-10张支持图像的少样本方法就达到了接近全监督基线的性能,而零样本方法通常在mIoU上落后15-20个百分点。
